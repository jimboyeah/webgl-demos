...
function App() {
  ...
  return (
    ...
    <Github pathname="src/WebInstanced.tsx" />
    set grid size:<br/>
    <input type="range" max="128" min="1" defaultValue={gridSize} value={state} title={`grid size ${state}x${state}`} 
      onChange={ ev => setstate(+ev.target.value)}/><br/>
    <input type="range" max="3200" min="1" defaultValue={gridSize} title={`grid size ${state}x${state}`} 
      onChange={ ev => setstate(+ev.target.value)}/><br/>
    </div>
